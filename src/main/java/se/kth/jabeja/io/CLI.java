...
public class CLI {
  ...
  public Config parseArgs(String[] args) throws FileNotFoundException {
    ...
    return new Config().setRandNeighborsSampleSize(randNeighborsSampleSize).setDelta(DELTA)
        ...
        .setSeed(SEED).setTemperature(TEMPERATURE).setTemperatureMin(TEMPERATUREMIN).setGraphFilePath(GRAPH)
        .setNodeSelectionPolicy(nodeSelectionPolicy).setGraphInitialColorPolicy(graphInitColorSelectionPolicy)
        .setOutputDir(OUTPUT_DIR).setAlpha(ALPHA);
  }
}
